...
class Theme extends \yii\base\Theme
{
    ...
    protected function autoFindModuleView($path)
    {
        ...
        // .../moduleId/views/controllerId/viewName.php
        if (preg_match('@.*' . $sep . '(.*?)' . $sep . 'views' . $sep . '(.*?)' . $sep . '(.*?)\.php$@', $path,
            $hits)) {
            return $this->getBasePath() . '/views/' . $hits[1] . '/' . $hits[2] . '/' . $hits[3] . '.php';
        }

        // /moduleId/[widgets|activities|notifications]/views/viewName.php
        if (preg_match('@.*' . $sep . '(.*?)' . $sep . '(widgets|notifications|activities)' . $sep . 'views' . $sep . '(.*?)\.php$@',
            $path, $hits)) {
    ...
    /**
     * Searches for a theme variable with the given $key.
     * @param string $key
     * @param mixed $default
     * @return string
     */
    ...
    /**
     * Parses the variables of the given less fileName.
     ...
     * @param string $lessFileName the less file to parse
     * @return array
     */
    public function parseThemeVariables($lessFileName)
    {
        ...
        return \yii\helpers\ArrayHelper::merge($variables,
            $this->parseLessVariables($this->getBasePath() . '/less/' . $lessFileName));
    }
